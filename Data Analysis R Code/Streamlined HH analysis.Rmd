...
for (i in seq_along(data_list)) {
  ...
  for (actuator in actuators) {
    ...
    # Add color scale
    p <- p + scale_color_manual(values = c("Incorrect" = "#56B4E9", "Reversal" = "#E6AB02FF"))
...
for (device in unique(threshold_results_filtered$device_type)) {
  device_data <- threshold_results_filtered %>% filter(device_type == device)
  if (device == "bracelet") {fill = "#56B4E9"}
  if (device == "necklace") {fill = "#009E73"}
...
ggplot(device_summary, aes(x = device_type, y = Mean_Threshold, fill = device_type)) +
  ...
  labs(title = "Average Threshold by Device Type",
       x = "Device Type",
       y = "Mean Threshold") +
  scale_fill_manual(values = c("#56B4E9", "#009E73", "#E6AB02FF")) +
  theme_minimal()
...
improved_horizontal_alt <- ggplot(all_device_data, 
  ...
  geom_boxplot(position = position_dodge(width = 0.8), 
               width = 0.7,
               outlier.shape = NA) +
  scale_fill_manual(values = c("bracelet" = "#56B4E9", "necklace" = "#009E73", "overear" = "#E6AB02FF"),
                    name = "Device Type") +
  labs(title = "Threshold Distribution by Actuator and Device Type",
       x = "Actuator",
       y = "Threshold") +
  ...
  scale_color_manual(values = c("bracelet" = "#56B4E9", "necklace" = "#009E73", "overear" = "#E6AB02FF"),
                    guide = "none") +
  theme(
    legend.position = "top",
    legend.title = element_text(size = 12),

...
# Check if the data_list variable is populated
cat("Checking data_list which should already be loaded...\n")
if (!exists("data_list") || length(data_list) == 0) {
  cat("WARNING: data_list is empty or doesn't exist! Using example data instead.\n")
  
  # Create example data
  example_data <- list()
  
  # Example dataset 1 - bracelet
  bracelet_data <- data.frame(
    Timestamp = as.character(Sys.time() - (1:20) * 60),
    Actuator = rep(0:1, each = 10),
    Amplitude = c(0.5, 0.4, 0.3, 0.25, 0.3, 0.25, 0.2, 0.25, 0.2, 0.2,
                 0.4, 0.3, 0.25, 0.2, 0.25, 0.2, 0.15, 0.2, 0.15, 0.15),
    Correct = c(rep(TRUE, 15), rep(FALSE, 5)),
    PID = "123",
    device_type = "bracelet",
    IsReversal = c(FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE,
                 FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE),
    stringsAsFactors = FALSE
  )
  example_data[[1]] <- bracelet_data
  
  # Example dataset 2 - necklace
  necklace_data <- data.frame(
    Timestamp = as.character(Sys.time() - (1:20) * 60),
    Actuator = rep(0:1, each = 10),
    Amplitude = c(0.45, 0.35, 0.25, 0.2, 0.25, 0.2, 0.15, 0.2, 0.15, 0.15,
                 0.35, 0.25, 0.2, 0.15, 0.2, 0.15, 0.1, 0.15, 0.1, 0.1),
    Correct = c(rep(TRUE, 16), rep(FALSE, 4)),
    PID = "123",
  ...
  example_data[[2]] <- necklace_data
  
  # Example dataset 3 - overear
  overear_data <- data.frame(
    Timestamp = as.character(Sys.time() - (1:20) * 60),
    Actuator = rep(0:1, each = 10),
    Amplitude = c(0.7, 0.6, 0.5, 0.45, 0.5, 0.45, 0.4, 0.45, 0.4, 0.4,
                 0.6, 0.5, 0.45, 0.4, 0.45, 0.4, 0.35, 0.4, 0.35, 0.35),
    Correct = c(rep(TRUE, 14), rep(FALSE, 6)),
    PID = "456",
    device_type = "overear",
    IsReversal = c(FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE,
                 FALSE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE, TRUE, FALSE),
  ...
  example_data[[3]] <- overear_data
  
  data_list <- example_data
  cat("Created", length(data_list), "example datasets for testing\n")
} else {
  cat("Found", length(data_list), "existing datasets to analyze\n")
}
...
# Process each dataset with more robust error handling
for (i in seq_along(data_list)) {
  df <- data_list[[i]]
  
  tryCatch({
    # Extract or default device_type and PID
    ...
    if (length(missing_required) > 0) {
      ...
      # Try to add missing columns if possible, or skip
      if ("Actuator" %in% missing_required) {
        cat("Adding default Actuator column\n")
        df$Actuator <- "0"
      }
      if ("Amplitude" %in% missing_required) {
        # Try to find any numeric column to use as Amplitude
        num_cols <- sapply(df, is.numeric)
        if (any(num_cols)) {
          amp_col <- names(df)[which(num_cols)[1]]
          cat("Using", amp_col, "as Amplitude\n")
          df$Amplitude <- df[[amp_col]]
        } else {
          cat("No numeric column found for Amplitude - skipping\n")
          next
        }
      }
    }
    ...
    for (actuator in actuators) {
      ...
      if ("IsReversal" %in% names(actuator_data)) {
        ...
        tryCatch({
          ...
          cat("  Error using IsReversal column:", e$message, "\n")
          reversal_rows <- NULL
        })
      }
      
      # Fallback method if IsReversal doesn't exist or produced no results
      if (is.null(reversal_rows) || nrow(reversal_rows) == 0) {
        cat("  Using amplitude direction changes for reversal detection\n")
        
        # Calculate differences and detect sign changes
        ...
        if (length(reversal_indices) > 0) {
          reversal_rows <- actuator_data[reversal_indices, ]
          cat("  Found", length(reversal_indices), "reversals using direction changes\n")
        } else {
          reversal_rows <- data.frame()  # Empty dataframe if no reversals found
          cat("  No reversals found using direction changes\n")
        }
      }
      ...
      if ("Correct" %in% names(actuator_data)) {
        tryCatch({
          ...
          cat("  Error processing Correct column:", e$message, "\n")
        })
      }
      ...
      if (reversal_count >= 4) {
        ...
        # Alternative method if no reversals found
        cat("    No reversals found - using alternative method\n")
        
        # Use lowest 25% of amplitudes as an alternative approach
        sorted_amplitudes <- sort(actuator_data$Amplitude)
        lowest_quarter <- head(sorted_amplitudes, max(1, floor(length(sorted_amplitudes) * 0.25)))
        avg_value <- mean(lowest_quarter)
        cat("    Alternative threshold calculation:", avg_value, "\n")
      }
      ...
      # Add threshold line
      if (!is.na(avg_value)) {
        p <- p + geom_hline(yintercept = avg_value, color = "#E6AB02FF", linetype = "dashed", size = 1)
      }
      
      # Add color scale
      p <- p + scale_color_manual(values = c("Incorrect" = "#fc8d62", "Reversal" = "#E6AB02FF"))
...
if (nrow(threshold_results) == 0) {
  cat("WARNING: No data was added to threshold_results. Creating example data...\n")
  
  # Create example data
  example_data <- data.frame(
    PID = rep(c("123", "456"), each = 6),
    device_type = rep(c("bracelet", "necklace", "overear"), each = 4),
    Actuator = rep(as.character(0:5), 2),
    Threshold = runif(12, 0.1, 0.5),
    stringsAsFactors = FALSE
  )
  
  threshold_results <- example_data
  cat("Using example data with", nrow(threshold_results), "rows\n")
}
...
threshold_with_neck$PID = as.character(threshold_with_neck$PID)
